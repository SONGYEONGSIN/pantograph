...
func TestFormatMismatchIsRejected(t *testing.T) {
	...
	if !errorsIs(err, diff.ErrFormatMismatch) {
...
**주의**: `errorsIs` 헬퍼를 손으로 쓰지 말고 표준 `errors.Is` 를 쓴다. 위 코드의 `errorsIs` 는 지우고 `"errors"` 를 import 해 `errors.Is(err, diff.ErrFormatMismatch)` 로 바꾼다.
