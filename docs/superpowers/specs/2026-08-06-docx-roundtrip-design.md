...
설계 초기의 실측은 `archive/zip`으로 했다 — `zip.File.OpenRaw()` → `zip.Writer.CreateRaw()`로 전 엔트리를 복사하면 원본과 sha256이 일치했고, DEFLATE와 STORE가 섞여 있어도 유지됐다.

**그러나 그 실측은 Python `zipfile`이 쓴 zip에 대한 것이었다.** 실제 Word 문서로 재검증하자 깨졌고, 그래서 지금은 `archive/zip`을 쓰지 않는다 — 컨테이너 파싱과 재조립을 직접 한다. 아래가 그 경위다.

**`archive/zip`이 보존하는 것은 엔트리의 *페이로드*지 *헤더*가 아니었다.** raw 통과 뒤에도 `Package.Write`가 로컬·중앙 헤더를 `zip.FileHeader`로부터 다시 찍어냈는데, `archive/zip`이 zip 헤더의 모든 것을 표현하지는 못한다. Info-ZIP이 쓴 zip으로 확인된 소실 두 가지:
...
**1번을 맨 앞에 뒀다.** 실제 Word docx에서 컨테이너 왕복이 깨지면 접근법의 토대가 무너지므로, 코드를 더 쌓기 전에 알아야 했다. Python이 쓴 zip에서 통과한 것은 증거가 아니었다 (§2.2).

실제로 깨졌다 — 다만 이 순서를 지켰음에도 픽스처가 늦게 도착해 슬라이스 끝에서 드러났다. 그때 컨테이너 자기검사 게이트가 이미 있었기 때문에 조용한 손상 대신 명시적 거절로 나타났고, 그것이 자체 zip 라이터로 이어졌다 (§2.2).
...
- **ZIP64** — 자체 라이터가 명시적으로 거절한다(§9). Word 16.x(macOS)가 쓴 픽스처는 ZIP64를 쓰지 않았으나, 4GiB 초과 문서나 65,535개 초과 파트에서 어떻게 되는지는 미확인이다
- **data descriptor** — 라이터가 다시 쓴다. Word 16.x 픽스처는 쓰지 않았고(`flags=0x0006`), 스트리밍 zip 라이터가 만든 docx로는 미검증이다
