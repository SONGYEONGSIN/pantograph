...
**패치·스키마 JSON 은 모르는 필드를 거절한다** (`DisallowUnknownFields`, 코드 1). `encoding/json` 의 기본값은 모르는 필드를 조용히 버리는 것이라, `text` 를 `value` 로 잘못 쓴 `setText` 가 빈 문자열로 성공했다 — `{"ok": true}` 를 내면서 그 자리의 텍스트를 지웠다. 빈 텍스트 자체는 정당한 연산이므로 결과만 봐서는 오타와 구별할 수 없고, 막을 수 있는 지점은 디코드뿐이다. 대가로 구버전 바이너리가 새 필드를 쓴 패치를 거절하게 되지만, **조용히 지우는 것보다 시끄럽게 거절하는 쪽이 이 도구의 계약(I1·I2)에 맞는다.**
