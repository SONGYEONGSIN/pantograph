...
**입력 JSON(패치·스키마·데이터)은 모르는 필드와 뒤에 남은 내용을 모두 거절한다** (`decodeStrict`, 코드 1). `encoding/json` 의 두 기본 동작이 각각 조용히 위험하다:

- `Unmarshal` 은 **모르는 필드를 버린다** — `text` 를 `value` 로 잘못 쓴 `setText` 가 빈 문자열로 성공했다. `{"ok": true}` 를 내면서 그 자리의 텍스트를 지웠다. 빈 텍스트 자체는 정당한 연산이라 결과만 봐서는 오타와 구별할 수 없고, 막을 수 있는 지점은 디코드뿐이다.
- `Decoder` 는 **첫 값만 읽고 뒤를 안 본다** — 잘리거나 두 번 이어붙은 파일이 통과한다. 뒤에 남은 내용은 `Token()` 이 `io.EOF` 를 내는지로 본다(`More()` 는 다음 바이트가 `]`·`}` 면 false 라 `{...}]` 를 놓친다).

**하나를 고치려고 다른 하나로 갈아타면 구멍이 옮겨갈 뿐이라** 두 검사를 한 함수에 함께 둔다. 대가로 구버전 바이너리가 새 필드를 쓴 패치를 거절하게 되지만, **조용히 지우는 것보다 시끄럽게 거절하는 쪽이 이 도구의 계약(I1·I2)에 맞는다.**
