...
func checkFields(op Op) *Error {
	...
	case "replaceRaw":
		...
		// 이쪽은 "다 쓰지 않고 끊었다"라 사용자가 할 일이 다르다.
		if err != nil {
			return &Error{Path: op.Path, Reason: "incomplete_xml",
				Detail: "replaceRaw 의 xml 이 끝까지 토큰화되지 않는다 — 조각 안에서 연 주석·CDATA·태그·속성값은 조각 안에서 끝내야 한다. 끊긴 조각은 뒤따르는 문서 바이트를 종결자로 삼아 대체 구간 밖의 내용을 삼킨다"}
		}
