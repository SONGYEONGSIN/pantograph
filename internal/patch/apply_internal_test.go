...
//
// empty_part 갈래는 이제 CLI 로는 도달할 수 없다 — replaceRaw 의 조각은 요소를
// 하나 이상 담아야 하고(checkFields), 루트 delete 는 delete_root 가 먼저 막는다.
// 그래도 백스톱은 남긴다: 내용을 지우는 경로의 마지막 그물이라 "지금 도달
// 불가"가 "불필요"를 뜻하지 않는다. 다만 아무도 시험할 수 없는 그물은 아무도
// 믿을 수 없으므로, 종단 경로 대신 술어를 여기서 시험한다.
